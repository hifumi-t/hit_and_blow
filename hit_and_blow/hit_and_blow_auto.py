...
def input_hidden(num_list) -> str:
    ...
    hidden_number = hidden_number[0]+hidden_number[1] + \
        hidden_number[2]+hidden_number[3]+hidden_number[4]
...
def post_hidden(room_id: int, i_am: str = i_am, URL: str = URL, session=session) -> str:
    headers = {"Content-Type": "application/json"}
    url_get_table = URL + "/rooms/" + \
        str(room_id) + "/players/" + i_am + "/hidden"
    ...
    while True:
        ...
        result_post = session.post(
            url_get_table, headers=headers, json=post_data)
...
def get_table(room_id: int, i_am: str = i_am, URL: str = URL, session=session) -> dict:
    url_get_table = URL + "/rooms/" + \
        str(room_id) + "/players/" + i_am + "/table"
...
def post_guess(room_id: int, guess_number: str, i_am: str = i_am, URL: str = URL, session=session):
    headers = {"Content-Type": "application/json"}
    url_post_guess = URL + "/rooms/" + \
        str(room_id) + "/players/" + i_am + "/table/guesses"
